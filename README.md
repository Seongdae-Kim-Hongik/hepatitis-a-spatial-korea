...
- contiguity graph of 223 districts (229 national districts minus 6 islands without a land neighbour); **219 districts × 5 years = 1,095 district-years** have complete, non-flagged covariates and enter the likelihood (Jeju-si and Seogwipo-si have no fiscal statistics of their own; Suwon-si and Seongnam-si have every study-year value of basic livelihood security recipients flagged by rule R7; Busanjin-gu's sewerage values, lost when the source was compiled, are restored by rule R0)
...
## Headline numbers (principal model M6, N = 1,095; `results/`)
- M6: DIC 5,648.21; WAIC 5,659.61; effective parameters 141.5. INLA fits of this model are not bit-reproducible: over the archived fit and 20 repeated fits with identical settings the DIC ranged 5,646.84–5,655.39 and the WAIC 5,658.71–5,663.02, and the credible set (the same 5 covariates) was identical in every fit (`results/repeated_fits.csv`)
- **5 credible covariates** (IRR = exponentiated posterior mean of the coefficient, per 1 SD of the transformed covariate; 95% CrI): household groundwater wells 1.078 (1.001–1.159); dairy-cattle farms 1.042 (1.003–1.083); sewer-pipe repair sites 0.906 (0.825–0.996); forest area 0.837 (0.757–0.926); fiscal independence 0.862 (0.790–0.940). Forest area, fiscal independence and sewer-pipe repair sites are credible under all 8 neighbourhood graphs; dairy-cattle farms under 7 and household groundwater wells under 6. Forest area, fiscal independence and dairy-cattle farms are credible under all 8 INLA settings; household groundwater wells and sewer-pipe repair sites under 7. Household groundwater wells and sewer-pipe repair sites are borderline
- not credible: piped water-supply coverage 1.019 (0.938–1.106); inpatient medical cost 1.063 (0.975–1.160); older adults living alone 0.962 (0.908–1.018); small-scale water-supply facilities 1.047 (0.984–1.113); food-security rate 1.031 (0.990–1.075); basic livelihood security recipients 1.004 (0.949–1.062); sex ratio 1.062 (0.992–1.137); no covariate outside the 5 is credible under any neighbourhood graph or INLA setting
...
- oyster production: the 5 associations are maintained when the annual score is replaced by a time-invariant producer indicator (under which the indicator itself, 0.947 (0.901–0.996), is also credible) or removed (`results/sens_oyster_coverage.csv`)
- sensitivity to the handling of extreme values (`results/sens_extreme_values.csv`): without rule R7 (the 17 flagged values retained, N = 1,105) all 5 associations remain credible, sewer-pipe repair sites only just (0.9098 (0.8279–0.9999)). With sewer-pipe repair sites log transformed, with or without R7, it is not credible (1.0125 (0.9763–1.0500); 1.0177 (0.9817–1.0550)) and neither is household groundwater wells (1.0679 (0.9916–1.1487); 1.0685 (0.9958–1.1455)); dairy-cattle farms, forest area and fiscal independence remain credible in all 3 analyses
- crude Moran's I = +0.738 (223 graph districts); +0.739 (219 analysed districts); residual Moran's I = +0.217 (count residuals, p < 0.001) and +0.112 (Pearson residuals, p = 0.004); fitted values include the random effects
- the covariates take up only part of the spatial structure: the posterior median SD of the structured spatial component is 0.29 without and 0.25 with the 27 covariates, the covariate-free model's DIC (5,651.74) and WAIC (5,663.63) differ from M6's by less than the variation between repeated fits, and the number of districts with credibly elevated/reduced effects is 78/59 without and 74/57 with covariates (219 analysed districts; `results/spatial_structure_with_without_covariates.csv`)
- Gi\* (223 districts, 95% level): 51 hot spots, 56 cold spots
- M4 (interaction without a temporal main effect) is numerically unstable — effective parameters 644–707 of 1,095, DIC 5,603.00–5,790.01 across fits — and is not used for model selection
...
| `results/analysis_dataset_compiled.csv` | The exact 1,095-row district-year analytic table (repaired, rules R0 and R7 applied) |
| `data_patches/busanjin_sewerage_restore.csv`, `results/data_patch_log.csv` | Busanjin-gu sewerage values restored by rule R0 and the log of what was restored |
...
| `results/district_effects.csv` | Posterior combined district effects (the 4 districts not analysed — Jeju-si, Seogwipo-si, Suwon-si, Seongnam-si — carry interpolated or prior-only effects and are not classified) |
...
### v2.3.0 — Busanjin-gu restored (2026-09-19)
The sewerage records of Busanjin-gu (Busan) were empty in every year of the compiled sewerage extracts, and the district had been excluded as having no recoverable sewerage data. The cause was a compilation step that removed a leading province abbreviation from district names: "부산진구" begins with "부산" (Busan), so it became "진구", failed to match the boundary file, and was written back as an all-zero row. Its values are now restored from an earlier compilation of the same Ministry of Environment sewerage statistics (`data_patches/busanjin_sewerage_restore.csv`, 53 values: coverage, septic-tank population, area, sewer-pipe repair sites). The 2023 values agree with the 2023 sewerage yearbook (population 361,996; coverage 100%; 84 repair sites) and the coverage series agrees with KOSIS DT_1YL20751 in every year 2016–2023. The earlier compilation matches the current extract in every other district except Jeju-si, whose earlier values are unweighted means of sub-district rows that do not match KOSIS and are therefore not used (Jeju-si remains excluded for lack of fiscal statistics). The restoration is applied in the script as rule R0, only where the source holds no non-zero value for that district and variable (`HAV_PATCH=false` to skip); `results/data_patch_log.csv` records it.
- **Analytic frame changed.** 218 districts / 1,090 district-years → **219 districts / 1,095 district-years** (13,992 cases).
- **Results.** The same 5 covariates are credible and every IRR changes only in the third decimal place. Household groundwater wells is now credible under 6 of 8 neighbourhood graphs and 7 of 8 INLA settings (5 and 6 before). Without rule R7, sewer-pipe repair sites is now just credible (0.9098, 0.8279–0.9999; previously just not credible), so its credibility no longer depends on R7. Residual Moran's I is +0.217.

### v2.2.1 — documentation correction (2026-09-19)
Documentation-only. The v2.2.0 release changed the principal analysis (rule R7, 218 districts, 1,090 district-years, 5 credible covariates) but left several v2.1.x figures uncorrected: the script header comment still described 220 districts, 1,100 district-years and 4 credible covariates, `DATA_PROVENANCE.md` still described the compiled analytic file as 1,100 rows / 220 districts, and this README's headline crude Moran's I (+0.735) was a truncated, mislabelled figure that matched neither the 223-district value (+0.738) nor the 218-district value (+0.736) reported in the manuscript's Table S4. All are corrected here. No change to code logic, data or results.
