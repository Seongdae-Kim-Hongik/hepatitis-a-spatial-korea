...
Usage: python3 qa/year_r2_before_after.py <v2.0.6 analysis_dataset_compiled.csv> results/analysis_dataset_compiled.csv results/year_r2_before_after.csv
"""
...
with open(sys.argv[3], "w", newline="") as f:
    w = csv.writer(f); w.writerow(["covariate", "year_r2_before_repair", "year_r2_after_repair"])
    for c in after: w.writerow([c, f"{before.get(c, float('nan')):.4f}", f"{after[c]:.4f}"])
